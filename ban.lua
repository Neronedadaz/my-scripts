...
local update_url = "https://raw.githubusercontent.com/Neronedadaz/my-scripts/main/update.ini" -- тут тоже свою ссылку
local update_path = getWorkingDirectory() .. "/update.ini" -- и тут свою ссылку

local script_url = "https://github.com/Neronedadaz/my-scripts/blob/main/ban.luac?raw=true" -- тут свою ссылку
...
wait(0)
 ...
 if update_state then
            downloadUrlToFile(script_url, script_path, function(id, status)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    sampShowDialog(126, 'Обновление', "{FFFFFF}Скрипт успешно обновлен!", 'Закрыть')
                    thisScript():reload()
                end
            end)
            break
...
imgui.Begin('ADMIN TOOLS | v.0.1 | Wolt_Nestero', main_window, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
